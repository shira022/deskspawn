...
| **バックエンド（Rust 側責務）** | ファイル I/O、XML パース・検証、プロセス管理（Vite / Tauri build）、許可リストによるシェル実行制御、キーチェーン保存 |
...
**最も重要なツール。** Bolt.diy 互換 XML を受け取り、Rust バックエンドにファイル操作を要求する。Rust 側で XML パース・検証・ファイル書き込み・自動連動（sqlx migrate / cargo check）が同期的に実行され、結果が返る。
...
- `<boltAction type="shell">` は許可リスト（`npm`, `npx`, `cargo`, `sqlx` のみ）に制限し、任意コマンド実行を防止
...
`apply_artifact` ツールに渡す XML の仕様。コードベースの成長に伴う破壊的上書きを防ぐため、**差分モード**と**変更プレビュー**を備える。
...
| モード | 属性 | 用途 | 安全性 |
|---|---|---|---|
| **file（全体）** | `<boltAction type="file" mode="file">` | 新規ファイル作成、または小規模ファイルの全置換 | 既存ロジック破壊リスクあり。変更プレビュー必須 |
| **diff（差分）** | `<boltAction type="file" mode="diff">` | 既存ファイルへの部分変更。`<search>` / `<replace>` ブロックで差分適用 | 高い。変更箇所のみ書き換え |
| **template** | `<boltAction type="template">` | テンプレートエンジンを起動し Rust コードを自動生成（3.2.7 参照） | 最高。AI は Rust コードを書かない |

###### 例1: 新規ファイル（mode="file"）

```xml
<boltArtifact id="new-component" title="新規コンポーネント追加">
  <boltAction type="file" mode="file" filePath="src/components/TaskList.tsx">
...
###### 例2: 差分適用（mode="diff"）— 推奨

```xml
<boltArtifact id="add-delete-button" title="削除ボタン追加">
  <boltAction type="file" mode="diff" filePath="src/components/TaskList.tsx">
    <search>
...
- `<search>`: ファイル内で**一意に**マッチするコードブロック（複数マッチの場合はエラー）
- `<replace>`: 置換後のコード。`<![CDATA[...]]>` によるエスケープを推奨
- マッチしない → エラーを `ApplyResult.errors` に返し、AI が再試行

###### 例3: テンプレート駆動（type="template"）

```xml
<boltArtifact id="crud-tasks" title="tasksテーブルのCRUD生成">
  <boltAction type="template" template="crud">
    tableName: tasks
    columns:
      - name: title
        sqlType: TEXT
...
| `<boltAction type="file" mode="file">` | `filePath` | ファイルの**全内容**を置換（新規 or 小規模ファイル用） |
| `<boltAction type="file" mode="diff">` | `filePath` | ファイル内の部分置換。`<search>` / `<replace>` 子要素必須 |
| `<boltAction type="template">` | `template` | テンプレート名（`crud` 等）。Rust コードはテンプレートエンジンが生成 |
| `<boltAction type="shell">` | （なし） | 実行すべきシェルコマンド。許可リスト検証後に実行 |
| `<search>` | （なし） | diff モード時の検索パターン（`mode="diff"` の子要素） |
| `<replace>` | （なし） | diff モード時の置換テキスト（`mode="diff"` の子要素） |

##### 制約

- 1つの `<boltArtifact>` 内で `mode="file"` / `mode="diff"` / `type="template"` は混在可能
- `mode="diff"` の `<search>` はファイル内で一意にマッチすること。複数マッチ → エラー
- `<boltAction type="shell">` は許可リスト（`npm`, `npx`, `cargo`, `sqlx`）のみ
- XML が不正な形式の場合、パースエラーを返し AI に再生成を要求

---

#### 3.2.4-bis 変更プレビューと適用フロー

コードの破壊的上書きを防ぐため、`apply_artifact` の適用前に**変更プレビュー**をユーザーに表示する。

##### フロー

```
apply_artifact(xml) が呼ばれる
    ↓
[1] Rust バックエンドが XML をパース・検証
    ↓
[2] 変更プレビューを生成（どのファイルのどの行が変更されるか）
    ↓
[3] チャットにプレビューを表示（diff 形式）:
    ┌─ 変更プレビュー ──────────────────────┐
    │ 📁 src/components/TaskList.tsx          │
    │   - 15: <li key={t.id}>{t.title}</li> │
    │   + 15: <li key={t.id}>               │
    │   + 16:   {t.title}                    │
    │   + 17:   <button>削除</button>        │
    │   + 18: </li>                          │
    │                                         │
    │ 📁 migrations/0001_create_tasks.sql (新規) │
    │   + CREATE TABLE tasks (...);           │
    ├────────────────────────────────────────┤
    │ [適用] [破棄]                            │
    └────────────────────────────────────────┘
    ↓
[4] ユーザーが「適用」→ ファイル反映 + 自動連動
    ユーザーが「破棄」→ 変更なし。AI に差し戻し
```

##### 自動適用ルール（ユーザー確認を省略可能なケース）

以下の条件を**すべて**満たす場合、プレビューを表示せず自動適用する：

- 変更が**新規ファイルのみ**（既存ファイルの変更ゼロ）
- かつ `<boltAction type="template">` によるテンプレート生成のみ
- かつ直前の `get_errors()` ですべてのエラーが解消された状態

それ以外のケース（既存ファイルの変更を含む、またはエラー状態での修正）では必ずプレビューを表示する。
...
```
XML パース（Rust側で検証）→ ファイル書き込み（Rust側でバックアップ）
    ↓
...
[1] ユーザー指示 → AI が TableDefinition（変数のみ）を抽出
    例: { tableName: "tasks", columns: [{ name: "title", sqlType: "TEXT", nullable: false }] }
         ↓
[2] AI が Bolt.diy XML で以下を出力:
    - SQL マイグレーションファイル (CREATE TABLE)
    - <boltAction type="template" template="crud" table="tasks" columns="...">
         ↓
[3] Rust バックエンドが <boltAction type="template"> を検出
...
| XML パースエラー | Rust 側 XML パーサー | `apply_artifact` の戻り値に即時反映 |
...
- [x] 単一プロジェクト管理
- [x] Vercel AI SDK エージェントループ × Bolt.diy 互換 XML（ファイル全体 / 差分 / テンプレート の3モード）
- [x] Node.js sidecar（AI 推論のみ）＋ Rust バックエンド（実実行）の責務分離アーキテクチャ
- [x] テンプレート駆動コード生成（CRUD 4パターン限定、型マッピング固定、テンプレートエンジンがRustコードを自動生成）
- [x] Bolt.diy XML 差分モード（`mode="diff"`、`<search>` / `<replace>`）＋ 変更プレビュー適用フロー
