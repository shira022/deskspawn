...
test('00: 初期状態 — クリア後は「アプリ未選択」のガイドが表示される', async () => {
  ...
  await page.screenshot({ path: '/home/<user>/hermes-project/project/deskspawn/e2e/screenshots/00-initial-state.png' });
});
...
test('01: 起動画面 — タイトルと主要UIが表示される', async () => {
  ...
  await page.screenshot({ path: '/home/<user>/hermes-project/project/deskspawn/e2e/screenshots/01-startup.png' });
});
...
test('02: AI設定フロー — プロバイダーを保存しツールバーに反映', async () => {
  ...
  await page.screenshot({ path: '/home/<user>/hermes-project/project/deskspawn/e2e/screenshots/02-provider-save.png' });
});
...
test('03: アプリ生成 — ToDoアプリを英語で作成してプレビュー表示', async () => {
  ...
  await page.screenshot({ path: '/home/<user>/hermes-project/project/deskspawn/e2e/screenshots/03-chat-response.png' });
});
...
test('04: 新規アプリ — ダイアログが開いてキャンセルできる', async () => {
  ...
  await expect(page.getByText('新しいアプリを作成', { exact: true })).toBeHidden();
  await page.screenshot({ path: '/home/<user>/hermes-project/project/deskspawn/e2e/screenshots/04-new-app-dialog.png' });
});
...
test('05: モデル設定メニュー — 現在のモデルが表示される', async () => {
  ...
  await closeModelPopover(page);
  await expect(popover).toBeHidden();
  await page.screenshot({ path: '/home/<user>/hermes-project/project/deskspawn/e2e/screenshots/05-model-settings.png' });
});
...
test('06: アプリ切替と削除 — 2アプリの作成・切替・削除ガード・後片付け', async () => {
  ...
  await page.screenshot({ path: '/home/<user>/hermes-project/project/deskspawn/e2e/screenshots/06-app-switch-delete.png' });
});
