...
```xml
<boltArtifact id="project" title="タスク管理アプリのDB追加">
  <boltAction type="file" filePath="src/App.tsx">
    import { useState, useEffect } from 'react';
    import { invoke } from '@tauri-apps/api/core';
    // ... 生成されたコード
  </boltAction>
  <boltAction type="file" filePath="migrations/20250101000000_create_tasks.sql">
  ...
  <boltAction type="file" filePath="src-tauri/src/db.rs">
    // sqlx クエリ定義を追記
    sqlx::query_as!(Task, "SELECT id, title, completed, created_at FROM tasks")
  </boltAction>
  <boltAction type="file" filePath="src-tauri/src/lib.rs">
    #[tauri::command]
    async fn get_tasks(state: State<'_, DbState>) -> Result<Vec<Task>, String> {
        // AIが生成したTauri command
    }
  </boltAction>
  <boltAction type="shell">
    sqlx migrate run && cargo check
  </boltAction>
</boltArtifact>
```
...
#### 3.2.7 AI コード生成テンプレート

Rust（sqlx + Tauri command）の自由生成は AI にとって難易度が高く、エラー率の上昇につながる。そのため、**テンプレート駆動のコード生成**を採用する。

##### テンプレート化の対象

| レイヤー | テンプレート方式 | AI が生成するもの |
|---|---|---|
| **DB モデル定義** | SQL マイグレーション（`migrations/*.sql`） | `CREATE TABLE` 文 |
| **Rust DB 層** | `db.rs` のテンプレート関数 | テーブル名・カラム名・型を埋め込んだ `sqlx::query_as!` 呼び出し |
| **Tauri Command** | `lib.rs` のテンプレートコマンド | `#[tauri::command]` 関数（CRUD パターン） |
| **React Hook** | テンプレートのカスタムフック | `invoke()` 呼び出しをラップした `useXxx()` フック |

##### テンプレート例: DB モデル → Tauri Command → React Hook

```
[人間の指示] 「タスク管理のDBを作って」

[AI が SQL マイグレーションを生成]
  → migrations/0001_create_tasks.sql
    CREATE TABLE tasks (id INTEGER PRIMARY KEY, title TEXT NOT NULL, ...)

[AI が Rust Tauri command を生成（テンプレートから埋め込み）]
...
- AI はテンプレートの穴埋めに集中でき、Rust の文法ミスが激減する
- 生成コードの一貫性が保たれ、デバッグが容易
- Tauri command の命名規則（`get_xxx`, `create_xxx`, `update_xxx`, `delete_xxx`）が統一される
...
- [x] テンプレート駆動コード生成（SQL→Tauri Command→React Hook）
