...
6. Check that the fix doesn't introduce new issues (lint, type errors)
...
- The same file/line is flagged in 3+ consecutive reviews for the same concern
- The fix agent cannot determine a viable approach after 2 attempts
...
- Fix in a SEPARATE session from review
...
- The fix log is append-only per iteration — each iteration gets its own file
