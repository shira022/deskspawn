...
不足時は送信をブロックし、設定导線を表示する（エラーレスポンスを待たずに阻止）。

### 2.3 環境チェック画面

Tauri で `.exe` をビルドするために必要な依存関係を自動検証する。**winget（Windows Package Manager）があればワンクリック自動セットアップが可能。**
...
| Rust (MSVC Toolchain) | `rustup show` / `rustc --version` | `Rustlang.Rustup` | 約400MB |
| Visual Studio Build Tools (MSVC) | レジストリ / `vswhere` 確認 | `Microsoft.VisualStudio.2022.BuildTools` | 約4.5GB（C++ ワークロード自動選択） |
| WebView2 Runtime | レジストリ確認 | `Microsoft.EdgeWebView2Runtime` | 約120MB |

> **macOS の場合**: VS Build Tools / WebView2 はスキップ（macOS では不要）

#### 自動セットアップフロー（winget 検出時）

1. **環境チェック（自動実行）**: 各依存関係をチェックし、✅ / ❌ で状態表示
...
4. **事前確認モーダル**:
   ...
   - VS Build Tools インストール時の大容量注意喚起
5. **インストール実行**:
   - 不足パッケージを順次 `winget install --id <package> --silent` で自動インストール
   - VS Build Tools は `--override` で C++ ワークロードを自動選択
   - プログレスバーで各パッケージの進捗をリアルタイム表示
...
- 「DeskSpawn を始める」ボタンは常に利用可能（一部不足でもチャット機能は利用可）
...
| イベント | タイミング | ペイロード |
|---|---|---|
| `step_progress` | 各ステップ開始時 | `{ step: number, maxSteps: number }` |
...
| 生成アプリのプレビュー | `5174` | Rust バックエンドが子プロセス起動 | プレビューパネルの iframe |

- **Rust バックエンド（Tauri 環境）**: workspace ディレクトリで `vite` を子プロセスとして起動し、空きポートを自動検出。検出したポートを `setWorkspacePort(port)` でフロントエンドに IPC 通知
- **ブラウザ開発モード（Tauri 非使用時）**: workspace Vite を開発者が手動起動（`npx vite --port 5174`）。フロントエンドは `workspacePort`（デフォルト 5174）を参照
