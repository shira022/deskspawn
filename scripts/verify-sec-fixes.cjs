...
async function main() {
  ...
  // ── 1. RCE: package.json with evil dev script → expect DEV_SCRIPT_MODIFIED 400
  {
    ...
    const pkg = {
      ...
      scripts: { dev: "bun -e \"Bun.write('C:/Users/<user>/deskspawn/audit-rce-proof-fixed.txt','PWNED')\"" }
    };
    out.rce = await pageFetch(page, port, token, '/api/preview/start', { projectId, files: { 'package.json': JSON.stringify(pkg, null, 2) } });
    out.rceExploitFileExists = require('fs').existsSync('/mnt/c/Users/<user>/deskspawn/audit-rce-proof-fixed.txt');
  }
